//////// Simple macro for producing analysis variables, stored in a root file and also plotted, for electron-neutrino Charged Current inclusive (nue CC) events
//////// This code loops into events produced by a GENIE simulation (using MicroBooNE flux), performs a electron-neutrino Charged Current inclusive event selection without energy or angular thresolds, and calculate some kinematical variables.
...
//////// CCDIS: charged current denergy_elecp inelastic scattering
...
////// true momentum of the true electron selected:

TH1D *truenergy_eleclectron_truemom_CCQE_GENIE;
TH1D *truenergy_eleclectron_truemom_CCMEC_GENIE;
TH1D *truenergy_eleclectron_truemom_CCRES_GENIE;
TH1D *truenergy_eleclectron_truemom_CCDIS_GENIE;
TH1D *truenergy_eleclectron_truemom_CCCOH_GENIE;
TH1D *truenergy_eleclectron_truemom_other_GENIE;

////// true cosine of theta (angle with respect to the neutrino beam direction) of the true electron selected

TH1D *truenergy_eleclectron_truecostheta_CCQE_GENIE;
TH1D *truenergy_eleclectron_truecostheta_CCMEC_GENIE;
TH1D *truenergy_eleclectron_truecostheta_CCRES_GENIE;
TH1D *truenergy_eleclectron_truecostheta_CCDIS_GENIE;
TH1D *truenergy_eleclectron_truecostheta_CCCOH_GENIE;
TH1D *truenergy_eleclectron_truecostheta_other_GENIE;

////// true phi angle of the true electron selected (nenergy_elecds to double check. I want it from -pi to pi)

TH1D *truenergy_eleclectron_truephi_CCQE_GENIE;
TH1D *truenergy_eleclectron_truephi_CCMEC_GENIE;
TH1D *truenergy_eleclectron_truephi_CCRES_GENIE;
TH1D *truenergy_eleclectron_truephi_CCDIS_GENIE;
TH1D *truenergy_eleclectron_truephi_CCCOH_GENIE;
TH1D *truenergy_eleclectron_truephi_other_GENIE;
...
///// missing transverse momentum (pT_miss), considering only a electron and protons are in the final state
...
///// ---- reconstructed neutrino energy in the Denergy_elecp Learning analysis for 1electron+ 1 proton selection ---
...
TH1D *Truenergy_elecnu_CCQE_GENIEnorw;
TH1D *Truenergy_elecnu_CCMEC_GENIEnorw;
TH1D *Truenergy_elecnu_CCRES_GENIEnorw;
TH1D *Truenergy_elecnu_CCDIS_GENIEnorw;
TH1D *Truenergy_elecnu_CCCOH_GENIEnorw;
TH1D *Truenergy_elecnu_other_GENIEnorw;

///// true neutrino energy, reweighted

TH1D *Truenergy_elecnu_CCQE_GENIE;
TH1D *Truenergy_elecnu_CCMEC_GENIE;
TH1D *Truenergy_elecnu_CCRES_GENIE;
TH1D *Truenergy_elecnu_CCDIS_GENIE;
TH1D *Truenergy_elecnu_CCCOH_GENIE;
TH1D *Truenergy_elecnu_other_GENIE;
...
int totentries =0;/// number of total nue CC events selected
int nCCQE = 0;/// number of total nue CCQE events selected
int nCCMEC =0;/// number of total nue CCMEC events selected
int nCCRes =0;/// number of total nue CCRes events selected
int nCCDIS =0;/// number of total nue CCDIS events selected
int nCCCOH =0;/// number of total nue CCCOH events selected
int nCCNue =0;/// number of total nue CCNue events selected (they aren't selected, this is not used)
int nNC =0;/// number of total nue neutral current events selected (they aren't selected, this is not used)
int nantinu= 0;/// number of total anti-nue events selected (they aren't selected, this is not used)
...
double GetEcalomiss(double Esum, double PTmiss, int np) { 
  ...
  double energy_elecxcit = 0.0304; //in GeV
  ...
  return Esum + energy_elecxcit + Ekinrecoil; // return result in GeV

}

///// formula of the reconstructed neutrino energy assuming it was a CCQE interaction and that the nucleon was at rest (proton mass does not correct for bound nucleon, we should do! but nenergy_elecd to get this number for argon first. Effect should be minimal). 

double GetEnuCCQE(double energy_elec, double cos_elec, double pelec){

  double Enum = (protonmass*energy_elec) - (emass*emass)/2.;
  double Eden = protonmass - energy_elec + (pelec*cos_elec);
...
double GetEnuRes(double energy_elec, double cos_elec){
  
  double mdelta= 1.232;//delta mass GeV/c2
  double Enum = (mdelta*mdelta) - (protonmass*protonmass) + (2.*protonmass*energy_elec);
  double Eden = 2.*(protonmass-(energy_elec*(1.-cos_elec)));
...
/// computing the phi angle /// ***** Nenergy_elecDS DOUBLE CHECK
...
double GetQ2(double enu, double energy_elec, double pelecz){ /// invariant 4-momentum transfer

  double pelecL = pelecz;
  double mom4 = 2 * enu * (energy_elec - pelecL) - emass*emass;
  return mom4;
...
double w(double energy_elec, double enu){ ///// energy transfer

  return (enu - energy_elec);
...
////////// topological description of the event using the particle content that escapes formt he nucleus after the interaction (final state particles). We will play with this later. Nenergy_elecds modifications forour own studies.

int Topology(int nelectrons, int nelectrons, int npions, int npi0, int nprotons) {
  //// This function return the true topology of the event, nue & anti-nue
  ////1. CC0Pion0Proton                                                                                                            
  ////2. CC0Pion1Proton                                  
  ////3. CC0Pion2Proton                                                                                                            
  ////4. CC0PionNProton                                                                                                            
  ...
  /// e.g. nue CC inclusive= Topology >0 && Topology < 7

  if (nelectrons >0 && (nelectrons + npions + npi0 ) == 0 && nprotons ==0 ) return 1;
  if (nelectrons >0 && (nelectrons + npions + npi0 ) == 0 && nprotons ==1 ) return 2;
  if (nelectrons >0 && (nelectrons + npions + npi0 ) == 0 && nprotons ==2 ) return 3;
  if (nelectrons >0 && (nelectrons + npions + npi0 ) == 0 && nprotons >2 ) return 4;
  if (nelectrons >0 && (nelectrons ) == 0 && (npions + npi0) == 1 ) return 5;
  if (nelectrons >0 && (nelectrons ) == 0 && (npions + npi0) > 1 ) return 6;
  //  if (nelectrons == 0 && nelectrons > 0 ) return 7;
  //  if (nelectrons==0  && nelectrons ==0) return 8;
...
//    ientry is the entry number in the current Trenergy_elec
//  Note that the argument to GetEntry must be:
//    jentry for TChain::GetEntry
//    ientry for TTrenergy_elec::GetEntry and TBranch::GetEntry
//
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  bool nueCC = false; //// by default we don't have any event selected as nue CC
  ...
  TTrenergy_elec *weights = (TTrenergy_elec*)uBtune->Get("weights");
  ...
  //// be careful when using TStack you nenergy_elecd all histograms with the same range and number of bins

  truenergy_eleclectron_truemom_CCQE_GENIE = new TH1D("truenergy_eleclectron_truemom_CCQE_GENIE", "truenergy_eleclectron_truemom_CCQE_GENIE", 60, 0, 3);
  truenergy_eleclectron_truemom_CCMEC_GENIE = new TH1D("truenergy_eleclectron_truemom_CCMEC_GENIE", "truenergy_eleclectron_truemom_CCMEC_GENIE", 60, 0, 3);
  truenergy_eleclectron_truemom_CCRES_GENIE = new TH1D("truenergy_eleclectron_truemom_CCRES_GENIE", "truenergy_eleclectron_truemom_CCRES_GENIE", 60, 0, 3);
  truenergy_eleclectron_truemom_CCDIS_GENIE = new TH1D("truenergy_eleclectron_truemom_CCDIS_GENIE", "truenergy_eleclectron_truemom_CCDIS_GENIE", 60, 0, 3);
  truenergy_eleclectron_truemom_CCCOH_GENIE = new TH1D("truenergy_eleclectron_truemom_CCCOH_GENIE", "truenergy_eleclectron_truemom_CCCOH_GENIE", 60, 0, 3);
  truenergy_eleclectron_truemom_other_GENIE = new TH1D("truenergy_eleclectron_truemom_other_GENIE", "truenergy_eleclectron_truemom_other_GENIE", 60, 0, 3);
 
  truenergy_eleclectron_truecostheta_CCQE_GENIE = new TH1D("truenergy_eleclectron_truecostheta_CCQE_GENIE", "truenergy_eleclectron_truecostheta_CCQE_GENIE", 60, -1, 1);
  truenergy_eleclectron_truecostheta_CCMEC_GENIE = new TH1D("truenergy_eleclectron_truecostheta_CCMEC_GENIE", "truenergy_eleclectron_truecostheta_CCMEC_GENIE", 60, -1, 1);
  truenergy_eleclectron_truecostheta_CCRES_GENIE = new TH1D("truenergy_eleclectron_truecostheta_CCRES_GENIE", "truenergy_eleclectron_truecostheta_CCRES_GENIE", 60, -1, 1);
  truenergy_eleclectron_truecostheta_CCDIS_GENIE = new TH1D("truenergy_eleclectron_truecostheta_CCDIS_GENIE", "truenergy_eleclectron_truecostheta_CCDIS_GENIE", 60, -1, 1);
  truenergy_eleclectron_truecostheta_CCCOH_GENIE = new TH1D("truenergy_eleclectron_truecostheta_CCCOH_GENIE", "truenergy_eleclectron_truecostheta_CCCOH_GENIE", 60, -1, 1);
  truenergy_eleclectron_truecostheta_other_GENIE = new TH1D("truenergy_eleclectron_truecostheta_other_GENIE", "truenergy_eleclectron_truecostheta_other_GENIE", 60, -1, 1);

  truenergy_eleclectron_truephi_CCQE_GENIE = new TH1D("truenergy_eleclectron_truephi_CCQE_GENIE", "truenergy_eleclectron_truephi_CCQE_GENIE", 60, -3.15, 3.15);
  truenergy_eleclectron_truephi_CCMEC_GENIE = new TH1D("truenergy_eleclectron_truephi_CCMEC_GENIE", "truenergy_eleclectron_truephi_CCMEC_GENIE", 60, -3.15, 3.15);
  truenergy_eleclectron_truephi_CCRES_GENIE = new TH1D("truenergy_eleclectron_truephi_CCRES_GENIE", "truenergy_eleclectron_truephi_CCRES_GENIE", 60, -3.15, 3.15);
  truenergy_eleclectron_truephi_CCDIS_GENIE = new TH1D("truenergy_eleclectron_truephi_CCDIS_GENIE", "truenergy_eleclectron_truephi_CCDIS_GENIE", 60, -3.15, 3.15);
  truenergy_eleclectron_truephi_CCCOH_GENIE = new TH1D("truenergy_eleclectron_truephi_CCCOH_GENIE", "truenergy_eleclectron_truephi_CCCOH_GENIE", 60, -3.15, 3.15);
  truenergy_eleclectron_truephi_other_GENIE = new TH1D("truenergy_eleclectron_truephi_other_GENIE", "truenergy_eleclectron_truephi_other_GENIE", 60, -3.15, 3.15);
  ...
  Truenergy_elecnu_CCQE_GENIE = new TH1D("Truenergy_elecnu_CCQE_GENIE", "Truenergy_elecnu_CCQE_GENIE", 60, 0, 3);
  Truenergy_elecnu_CCMEC_GENIE = new TH1D("Truenergy_elecnu_CCMEC_GENIE", "Truenergy_elecnu_CCMEC_GENIE", 60, 0, 3);
  Truenergy_elecnu_CCRES_GENIE = new TH1D("Truenergy_elecnu_CCRES_GENIE", "Truenergy_elecnu_CCRES_GENIE", 60, 0, 3);
  Truenergy_elecnu_CCDIS_GENIE = new TH1D("Truenergy_elecnu_CCDIS_GENIE", "Truenergy_elecnu_CCDIS_GENIE", 60, 0, 3);
  Truenergy_elecnu_CCCOH_GENIE = new TH1D("Truenergy_elecnu_CCCOH_GENIE", "Truenergy_elecnu_CCCOH_GENIE", 60, 0, 3);
  Truenergy_elecnu_other_GENIE = new TH1D("Truenergy_elecnu_other_GENIE", "Truenergy_elecnu_other_GENIE", 60, 0, 3);

  Truenergy_elecnu_CCQE_GENIEnorw = new TH1D("Truenergy_elecnu_CCQE_GENIEnorw", "Truenergy_elecnu_CCQE_GENIEnorw", 60, 0, 3);
  Truenergy_elecnu_CCMEC_GENIEnorw = new TH1D("Truenergy_elecnu_CCMEC_GENIEnorw", "Truenergy_elecnu_CCMEC_GENIEnorw", 60, 0, 3);
  Truenergy_elecnu_CCRES_GENIEnorw = new TH1D("Truenergy_elecnu_CCRES_GENIEnorw", "Truenergy_elecnu_CCRES_GENIEnorw", 60, 0, 3);
  Truenergy_elecnu_CCDIS_GENIEnorw = new TH1D("Truenergy_elecnu_CCDIS_GENIEnorw", "Truenergy_elecnu_CCDIS_GENIEnorw", 60, 0, 3);
  Truenergy_elecnu_CCCOH_GENIEnorw = new TH1D("Truenergy_elecnu_CCCOH_GENIEnorw", "Truenergy_elecnu_CCCOH_GENIEnorw", 60, 0, 3);
  Truenergy_elecnu_other_GENIEnorw = new TH1D("Truenergy_elecnu_other_GENIEnorw", "Truenergy_elecnu_other_GENIEnorw", 60, 0, 3);
...
///////////////////////////////////////////
  ...
  /////// and here starts the real loop event by event
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     Long64_t ientry = LoadTrenergy_elec(jentry);
      if (ientry < 0) break;
      ...
      nueCC = false; /// re-initiate the boolean to false, so we don't select  events by mistake

      if(neu == 14 && cc && Ev>0 && El >0) nueCC = true; /// this is the condition to select a nue CC event in true variables (neu== neutrino pdg is for electron neutrino, cc== is a charged current event, Ev== neutrino energy, El==charged lepton energy). Boolean is TRUE if the event is nue CC
      if (!nueCC) continue;
      
      totentries++; /// counting number of events selected as nue CC
      ...
      double deltax =pxl;/// electron px (momentum in x)
      double deltay =pyl; // electron py (momentum in y)
      double Esum =0; /// sum of total kinetic energies
      double pelec = sqrt(pxl*pxl + pyl*pyl + pzl*pzl); /// true electron (absolute) momentum
      double energy_elec = sqrt(emass*emass + pelec*pelec); /// true electron energy
      double q2 = GetQ2(Ev, energy_elec, pzl); /// 4-momentum transfer from true kinematics
      double omega = w(energy_elec, Ev); /// true energy transfer
	...
	if (pdgf[ki] == 2212) {/// we look at each particle type using their PDG codes (https://pdg.lbl.gov/2007/reviews/montecarlorpp.pdf)
      ...
      //////////  end loop final state hadrons ///////////////
       ...
       Esum= Esum + energy_elec; /// total energy of the electron + protons kinetic energy
	 ...
	 //// true kinematics /////
	 truenergy_eleclectron_truemom_CCQE_GENIE->Fill(sqrt(pow(pxl,2)+ pow(pyl,2)+pow(pzl,2)), rw);
	 truenergy_eleclectron_truecostheta_CCQE_GENIE->Fill(cosine(pxl, pyl , pzl),rw);
	 truenergy_eleclectron_truephi_CCQE_GENIE->Fill(Getphi(pxl, pyl, pzl),rw);
	 ...
	 pT_CCQE_GENIE->Fill(PTmiss,rw);/// assuming 1electron + protons in the final state
	 EnuCCQE_CCQE_GENIE->Fill(GetEnuCCQE(energy_elec, cosine(pxl, pyl , pzl), pelec),rw);
	 EnuCalo_CCQE_GENIE->Fill(GetEcalomiss(Esum, PTmiss, 0),rw);/// assuming 1electron + protons int eh final state, Ar40 (np=0)
	 EnuRes_CCQE_GENIE->Fill(GetEnuRes(energy_elec, cosine(pxl, pyl , pzl)),rw);
	 Truenergy_elecnu_CCQE_GENIEnorw->Fill(EvRF);
	 Truenergy_elecnu_CCQE_GENIE->Fill(EvRF,rw);
	 ...
	 //// 1electron+1 proton //// Denergy_elecp Learning signal
	 ...
	 Resolution_EnuCCQE_GENIE->Fill(EvRF,EvRF - GetEnuCCQE(energy_elec, cosine(pxl, pyl , pzl), pelec),rw);
	 Resolution_EnuCalo_GENIE->Fill(EvRF,EvRF - GetEcalomiss(Esum, PTmiss, 0),rw);
	 Resolution_EnuRes_GENIE->Fill(EvRF,EvRF - GetEnuRes(energy_elec, cosine(pxl, pyl , pzl)),rw);

	 hprof_EnuCCQE_GENIE->Fill(EvRF,EvRF - GetEnuCCQE(energy_elec, cosine(pxl, pyl , pzl), pelec),rw);
	 hprof_EnuCalo_GENIE->Fill(EvRF,EvRF - GetEcalomiss(Esum, PTmiss, 0),rw);
	 hprof_EnuRes_GENIE->Fill(EvRF,EvRF - GetEnuRes(energy_elec, cosine(pxl, pyl , pzl)),rw);
	 
	 hprof_EnuCCQE_GENIEnorw->Fill(EvRF,EvRF - GetEnuCCQE(energy_elec, cosine(pxl, pyl , pzl), pelec),1);
	 hprof_EnuCalo_GENIEnorw->Fill(EvRF,EvRF - GetEcalomiss(Esum, PTmiss, 0),1);
	 hprof_EnuRes_GENIEnorw->Fill(EvRF,EvRF - GetEnuRes(energy_elec, cosine(pxl, pyl , pzl)),1);
	 ...
	 /// true kinematics /////
	 truenergy_eleclectron_truemom_CCMEC_GENIE->Fill(sqrt(pow(pxl,2)+ pow(pyl,2)+pow(pzl,2)),rw);
	 truenergy_eleclectron_truecostheta_CCMEC_GENIE->Fill(cosine(pxl, pyl , pzl),rw);
	 truenergy_eleclectron_truephi_CCMEC_GENIE->Fill(Getphi(pxl, pyl, pzl),rw);
	 ...
	 pT_CCMEC_GENIE->Fill(PTmiss,rw);/// assuming 1electron + protons int he final state
	 EnuCCQE_CCMEC_GENIE->Fill(GetEnuCCQE(energy_elec, cosine(pxl, pyl , pzl), pelec),rw);
	 EnuCalo_CCMEC_GENIE->Fill(GetEcalomiss(Esum, PTmiss, 0),rw);/// assuming 1electron + protons int eh final state, Ar40 (np=0)
	 EnuRes_CCMEC_GENIE->Fill(GetEnuRes(energy_elec, cosine(pxl, pyl , pzl)),rw);
	 Truenergy_elecnu_CCMEC_GENIEnorw->Fill(EvRF);
	 Truenergy_elecnu_CCMEC_GENIE->Fill(EvRF,rw);
	...
	////// true kinematics /////
	 truenergy_eleclectron_truemom_CCRES_GENIE->Fill(sqrt(pow(pxl,2)+ pow(pyl,2)+pow(pzl,2)),rw);
	 truenergy_eleclectron_truecostheta_CCRES_GENIE->Fill(cosine(pxl, pyl , pzl),rw);
	 truenergy_eleclectron_truephi_CCRES_GENIE->Fill(Getphi(pxl, pyl, pzl),rw);
	 ...
	 pT_CCRES_GENIE->Fill(PTmiss,rw);/// assuming 1electron + protons int he final state
	 EnuCCQE_CCRES_GENIE->Fill(GetEnuCCQE(energy_elec, cosine(pxl, pyl , pzl), pelec),rw);
	 EnuCalo_CCRES_GENIE->Fill(GetEcalomiss(Esum, PTmiss, 0),rw);/// assuming 1electron + protons int eh final state, Ar40 (np=0)
	 EnuRes_CCRES_GENIE->Fill(GetEnuRes(energy_elec, cosine(pxl, pyl , pzl)),rw);
	 Truenergy_elecnu_CCRES_GENIEnorw->Fill(EvRF);
	 Truenergy_elecnu_CCRES_GENIE->Fill(EvRF,rw);
	 ...
	 //// true kinematics /////
	 truenergy_eleclectron_truemom_CCDIS_GENIE->Fill(sqrt(pow(pxl,2)+ pow(pyl,2)+pow(pzl,2)),rw);
	 truenergy_eleclectron_truecostheta_CCDIS_GENIE->Fill(cosine(pxl, pyl , pzl),rw);
	 truenergy_eleclectron_truephi_CCDIS_GENIE->Fill(Getphi(pxl, pyl, pzl),rw);
	 ...
	 pT_CCDIS_GENIE->Fill(PTmiss,rw);/// assuming 1electron + protons int he final state
	 EnuCCQE_CCDIS_GENIE->Fill(GetEnuCCQE(energy_elec, cosine(pxl, pyl , pzl), pelec),rw);
	 EnuCalo_CCDIS_GENIE->Fill(GetEcalomiss(Esum, PTmiss, 0),rw);/// assuming 1electron + protons int eh final state, Ar40 (np=0)
	 EnuRes_CCDIS_GENIE->Fill(GetEnuRes(energy_elec, cosine(pxl, pyl , pzl)),rw);
	 Truenergy_elecnu_CCDIS_GENIEnorw->Fill(EvRF);
	 Truenergy_elecnu_CCDIS_GENIE->Fill(EvRF,rw);
	...
	////// filling resolution histograms in all interaction types
	 ...
	 /// true kinematics /////
	 truenergy_eleclectron_truemom_CCCOH_GENIE->Fill(sqrt(pow(pxl,2)+ pow(pyl,2)+pow(pzl,2)),rw);
	 truenergy_eleclectron_truecostheta_CCCOH_GENIE->Fill(cosine(pxl, pyl , pzl),rw);
	 truenergy_eleclectron_truephi_CCCOH_GENIE->Fill(Getphi(pxl, pyl, pzl),rw);
	 ...
	 pT_CCCOH_GENIE->Fill(PTmiss,rw);/// assuming 1electron + protons int he final state
	 EnuCCQE_CCCOH_GENIE->Fill(GetEnuCCQE(energy_elec, cosine(pxl, pyl , pzl), pelec),rw);
	 EnuCalo_CCCOH_GENIE->Fill(GetEcalomiss(Esum, PTmiss, 0),rw);/// assuming 1electron + protons int eh final state, Ar40 (np=0)
	 EnuRes_CCCOH_GENIE->Fill(GetEnuRes(energy_elec, cosine(pxl, pyl , pzl)),rw);
	 Truenergy_elecnu_CCCOH_GENIEnorw->Fill(EvRF);
	 Truenergy_elecnu_CCCOH_GENIE->Fill(EvRF,rw);
	...
	///// true kinematics /////
	 truenergy_eleclectron_truemom_other_GENIE->Fill(sqrt(pow(pxl,2)+ pow(pyl,2)+pow(pzl,2)),rw);
	 truenergy_eleclectron_truecostheta_other_GENIE->Fill(cosine(pxl, pyl , pzl),rw);
	 truenergy_eleclectron_truephi_other_GENIE->Fill(Getphi(pxl, pyl, pzl),rw);
	 ...
	 pT_other_GENIE->Fill(PTmiss,rw);/// assuming 1electron + protons int he final state
	 EnuCCQE_other_GENIE->Fill(GetEnuCCQE(energy_elec, cosine(pxl, pyl , pzl), pelec),rw);
	 EnuCalo_other_GENIE->Fill(GetEcalomiss(Esum, PTmiss, 0),rw);/// assuming 1electron + protons int eh final state, Ar40 (np=0)
	 EnuRes_other_GENIE->Fill(GetEnuRes(energy_elec, cosine(pxl, pyl , pzl)),rw);
	 Truenergy_elecnu_other_GENIEnorw->Fill(EvRF);
	 Truenergy_elecnu_other_GENIE->Fill(EvRF,rw);
  ...
  //  gStyle->SetLabelOffset(0.015,"Y");
   ...
   /// each plot is stored as pdf, eps png and also in .C (sometimes helps when one cannot re-run the entire code but nenergy_elecds to change any style)
   /// some legends are misplaced, nenergy_elecd to check one-by-one
   
  TCanvas *c1 = new TCanvas("c1", "c1", 900, 900);
  
  truenergy_eleclectron_truemom_CCQE_GENIE-> SetFillColor(kRed);
  truenergy_eleclectron_truemom_CCMEC_GENIE-> SetFillColor(kGrenergy_elecn);
  truenergy_eleclectron_truemom_CCRES_GENIE-> SetFillColor(kYellow);
  truenergy_eleclectron_truemom_CCDIS_GENIE-> SetFillColor(kBlue);
  truenergy_eleclectron_truemom_CCCOH_GENIE-> SetFillColor(kMagenta);
 
  THStack *truenergy_eleclectron_truemom_GENIE = new THStack("truenergy_eleclectron_truemom_GENIE","");
  truenergy_eleclectron_truemom_GENIE-> Add(truenergy_eleclectron_truemom_CCQE_GENIE);
  truenergy_eleclectron_truemom_GENIE-> Add(truenergy_eleclectron_truemom_CCMEC_GENIE);
  truenergy_eleclectron_truemom_GENIE-> Add(truenergy_eleclectron_truemom_CCRES_GENIE);
  truenergy_eleclectron_truemom_GENIE-> Add(truenergy_eleclectron_truemom_CCDIS_GENIE);
  truenergy_eleclectron_truemom_GENIE-> Add(truenergy_eleclectron_truemom_CCCOH_GENIE);
  //truenergy_eleclectron_truemom_GENIE-> SetMaximum(100);
  truenergy_eleclectron_truemom_GENIE-> Draw("hist");
  truenergy_eleclectron_truemom_GENIE->GetXaxis()->SetTitle("p_{#elec} [GeV/c]");
  truenergy_eleclectron_truemom_GENIE->GetYaxis()->SetTitle("number of events");
  
  TLegend *l1 = new TLegend(0.5, 0.7, 0.9, 0.9);
  l1 -> AddEntry(truenergy_eleclectron_truemom_CCQE_GENIE, "CCQE", "f");
  l1 -> AddEntry(truenergy_eleclectron_truemom_CCMEC_GENIE, "CCMEC", "f");
  l1 -> AddEntry(truenergy_eleclectron_truemom_CCRES_GENIE, "CCRES", "f");
  l1 -> AddEntry(truenergy_eleclectron_truemom_CCCOH_GENIE, "CC-Coh", "f");
  l1 -> AddEntry(truenergy_eleclectron_truemom_CCDIS_GENIE, "CCDIS", "f");
  l1 -> Draw();
  
  c1->Update();
  c1->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/truenergy_eleclectron_truemom_GENIErw.pdf");
  c1->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/truenergy_eleclectron_truemom_GENIErw.eps");
  c1->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/truenergy_eleclectron_truemom_GENIErw.png");
  c1->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/truenergy_eleclectron_truemom_GENIErw.C");
  
  TCanvas *c2 = new TCanvas("c2", "c2", 900, 900);
  
  truenergy_eleclectron_truecostheta_CCQE_GENIE-> SetFillColor(kRed);
  truenergy_eleclectron_truecostheta_CCMEC_GENIE-> SetFillColor(kGrenergy_elecn);
  truenergy_eleclectron_truecostheta_CCRES_GENIE-> SetFillColor(kYellow);
  truenergy_eleclectron_truecostheta_CCDIS_GENIE-> SetFillColor(kBlue);
  truenergy_eleclectron_truecostheta_CCCOH_GENIE-> SetFillColor(kMagenta);
 
  THStack *truenergy_eleclectron_truecostheta_GENIE = new THStack("truenergy_eleclectron_truecostheta_GENIE","");
  truenergy_eleclectron_truecostheta_GENIE-> Add(truenergy_eleclectron_truecostheta_CCQE_GENIE);
  truenergy_eleclectron_truecostheta_GENIE-> Add(truenergy_eleclectron_truecostheta_CCMEC_GENIE);
  truenergy_eleclectron_truecostheta_GENIE-> Add(truenergy_eleclectron_truecostheta_CCRES_GENIE);
  truenergy_eleclectron_truecostheta_GENIE-> Add(truenergy_eleclectron_truecostheta_CCDIS_GENIE);
  truenergy_eleclectron_truecostheta_GENIE-> Add(truenergy_eleclectron_truecostheta_CCCOH_GENIE);
  //truenergy_eleclectron_truemom_GENIE-> SetMaximum(100);
  truenergy_eleclectron_truecostheta_GENIE-> Draw("hist");
  truenergy_eleclectron_truecostheta_GENIE->GetXaxis()->SetTitle("cos #theta_{#elec}");
  truenergy_eleclectron_truecostheta_GENIE->GetYaxis()->SetTitle("number of events");

  TLegend *l2 = new TLegend(0.5, 0.7, 0.9, 0.9);
  l2 -> AddEntry(truenergy_eleclectron_truecostheta_CCQE_GENIE, "CCQE", "f");
  l2 -> AddEntry(truenergy_eleclectron_truecostheta_CCMEC_GENIE, "CCMEC", "f");
  l2 -> AddEntry(truenergy_eleclectron_truecostheta_CCRES_GENIE, "CCRES", "f");
  l2 -> AddEntry(truenergy_eleclectron_truecostheta_CCCOH_GENIE, "CC-Coh", "f");
  l2 -> AddEntry(truenergy_eleclectron_truecostheta_CCDIS_GENIE, "CCDIS", "f");
  l2 -> Draw();

  c2->Update();
  c2->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/truenergy_eleclectron_truecostheta_GENIErw.pdf");
  c2->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/truenergy_eleclectron_truecostheta_GENIErw.eps");
  c2->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/truenergy_eleclectron_truecostheta_GENIErw.png");
  c2->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/truenergy_eleclectron_truecostheta_GENIErw.C");

  TCanvas *c3 = new TCanvas("c3", "c3", 900, 900);
  
  truenergy_eleclectron_truephi_CCQE_GENIE-> SetFillColor(kRed);
  truenergy_eleclectron_truephi_CCMEC_GENIE-> SetFillColor(kGrenergy_elecn);
  truenergy_eleclectron_truephi_CCRES_GENIE-> SetFillColor(kYellow);
  truenergy_eleclectron_truephi_CCDIS_GENIE-> SetFillColor(kBlue);
  truenergy_eleclectron_truephi_CCCOH_GENIE-> SetFillColor(kMagenta);
 
  THStack *truenergy_eleclectron_truephi_GENIE = new THStack("truenergy_eleclectron_truephi_GENIE","");
  truenergy_eleclectron_truephi_GENIE-> Add(truenergy_eleclectron_truephi_CCQE_GENIE);
  truenergy_eleclectron_truephi_GENIE-> Add(truenergy_eleclectron_truephi_CCMEC_GENIE);
  truenergy_eleclectron_truephi_GENIE-> Add(truenergy_eleclectron_truephi_CCRES_GENIE);
  truenergy_eleclectron_truephi_GENIE-> Add(truenergy_eleclectron_truephi_CCDIS_GENIE);
  truenergy_eleclectron_truephi_GENIE-> Add(truenergy_eleclectron_truephi_CCCOH_GENIE);
  //truenergy_eleclectron_truemom_GENIE-> SetMaximum(100);
  truenergy_eleclectron_truephi_GENIE-> Draw("hist");
  truenergy_eleclectron_truephi_GENIE->GetXaxis()->SetTitle("#phi_{#elec}");
  truenergy_eleclectron_truephi_GENIE->GetYaxis()->SetTitle("number of events");

  TLegend *l3 = new TLegend(0.5, 0.7, 0.9, 0.9);
  l3 -> AddEntry(truenergy_eleclectron_truephi_CCQE_GENIE, "CCQE", "f");
  l3 -> AddEntry(truenergy_eleclectron_truephi_CCMEC_GENIE, "CCMEC", "f");
  l3 -> AddEntry(truenergy_eleclectron_truephi_CCRES_GENIE, "CCRES", "f");
  l3 -> AddEntry(truenergy_eleclectron_truephi_CCCOH_GENIE, "CC-Coh", "f");
  l3 -> AddEntry(truenergy_eleclectron_truephi_CCDIS_GENIE, "CCDIS", "f");
  l3 -> Draw();

  c3->Update();
  c3->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/truenergy_eleclectron_truephi_GENIErw.pdf");
  c3->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/truenergy_eleclectron_truephi_GENIErw.eps");
  c3->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/truenergy_eleclectron_truephi_GENIErw.png");
  c3->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/truenergy_eleclectron_truephi_GENIErw.C");

  TCanvas *c4 = new TCanvas("c4", "c4", 900, 900);
  
  trueNh_CCQE_GENIE-> SetFillColor(kRed);
  trueNh_CCMEC_GENIE-> SetFillColor(kGrenergy_elecn);
  ...
  trueNvish_CCMEC_GENIE-> SetFillColor(kGrenergy_elecn);
  ...
  trueW_CCMEC_GENIE-> SetFillColor(kGrenergy_elecn);
  ...
  pT_CCMEC_GENIE-> SetFillColor(kGrenergy_elecn);
  ...
  EnuCCQE_CCMEC_GENIE-> SetFillColor(kGrenergy_elecn);
  ...
  EnuCalo_CCMEC_GENIE-> SetFillColor(kGrenergy_elecn);
  ...
  EnuRes_CCMEC_GENIE-> SetFillColor(kGrenergy_elecn);
  ...
  hprof_EnuRes_GENIE->SetLineColor(kGrenergy_elecn);
  hprof_EnuRes_GENIE->SetLineWidth(2);
  hprof_EnuRes_GENIE->SetMarkerColor(kGrenergy_elecn);
  ...
  Truenergy_elecnu_CCQE_GENIE-> SetFillColor(kRed);
  Truenergy_elecnu_CCMEC_GENIE-> SetFillColor(kGrenergy_elecn);
  Truenergy_elecnu_CCRES_GENIE-> SetFillColor(kYellow);
  Truenergy_elecnu_CCDIS_GENIE-> SetFillColor(kBlue);
  Truenergy_elecnu_CCCOH_GENIE-> SetFillColor(kMagenta);
 
  THStack *Truenergy_elecnu_GENIE = new THStack("Truenergy_elecnu_GENIE","");
  Truenergy_elecnu_GENIE-> Add(Truenergy_elecnu_CCQE_GENIE);
  Truenergy_elecnu_GENIE-> Add(Truenergy_elecnu_CCMEC_GENIE);
  Truenergy_elecnu_GENIE-> Add(Truenergy_elecnu_CCRES_GENIE);
  Truenergy_elecnu_GENIE-> Add(Truenergy_elecnu_CCDIS_GENIE);
  Truenergy_elecnu_GENIE-> Add(Truenergy_elecnu_CCCOH_GENIE); 
  //truenergy_eleclectron_truemom_GENIE-> SetMaximum(100);
  Truenergy_elecnu_GENIE-> Draw("hist");
  Truenergy_elecnu_GENIE->GetXaxis()->SetTitle("reconstructed neutrino energy in resonant approach [GeV/c]");
  Truenergy_elecnu_GENIE->GetYaxis()->SetTitle("number of events");

  TLegend *l15 = new TLegend(0.5, 0.7, 0.9, 0.9);
  l15 -> AddEntry(Truenergy_elecnu_CCQE_GENIE, "CCQE", "f");
  l15 -> AddEntry(Truenergy_elecnu_CCMEC_GENIE, "CCMEC", "f");
  l15 -> AddEntry(Truenergy_elecnu_CCRES_GENIE, "CCRES", "f");
  l15 -> AddEntry(Truenergy_elecnu_CCCOH_GENIE, "CC-Coh", "f");
  l15 -> AddEntry(Truenergy_elecnu_CCDIS_GENIE, "CCDIS", "f");
  l15 -> Draw();

  c15->Update();
  c15->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/Truenergy_elecnu_GENIE.pdf");
  c15->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/Truenergy_elecnu_GENIE.eps");
  c15->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/Truenergy_elecnu_GENIE.png");
  c15->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/Truenergy_elecnu_GENIE.C");  

  TCanvas *c16 = new TCanvas("c16", "c16", 900, 900);
  
  Truenergy_elecnu_CCQE_GENIEnorw-> SetFillColor(kRed);
  Truenergy_elecnu_CCMEC_GENIEnorw-> SetFillColor(kGrenergy_elecn);
  Truenergy_elecnu_CCRES_GENIEnorw-> SetFillColor(kYellow);
  Truenergy_elecnu_CCDIS_GENIEnorw-> SetFillColor(kBlue);
  Truenergy_elecnu_CCCOH_GENIEnorw-> SetFillColor(kMagenta);
 
  THStack *Truenergy_elecnu_GENIEnorw = new THStack("Truenergy_elecnu_GENIEnorw","");
  Truenergy_elecnu_GENIEnorw-> Add(Truenergy_elecnu_CCQE_GENIEnorw);
  Truenergy_elecnu_GENIEnorw-> Add(Truenergy_elecnu_CCMEC_GENIEnorw);
  Truenergy_elecnu_GENIEnorw-> Add(Truenergy_elecnu_CCRES_GENIEnorw);
  Truenergy_elecnu_GENIEnorw-> Add(Truenergy_elecnu_CCDIS_GENIEnorw);
  Truenergy_elecnu_GENIEnorw-> Add(Truenergy_elecnu_CCCOH_GENIEnorw); 
  Truenergy_elecnu_GENIEnorw-> Draw("hist");
  Truenergy_elecnu_GENIEnorw->GetXaxis()->SetTitle("reconstructed neutrino energy in resonant approach [GeV/c]");
  Truenergy_elecnu_GENIEnorw->GetYaxis()->SetTitle("number of events");

  TLegend *l16 = new TLegend(0.5, 0.7, 0.9, 0.9);
  l16 -> AddEntry(Truenergy_elecnu_CCQE_GENIEnorw, "CCQE", "f");
  l16 -> AddEntry(Truenergy_elecnu_CCMEC_GENIEnorw, "CCMEC", "f");
  l16 -> AddEntry(Truenergy_elecnu_CCRES_GENIEnorw, "CCRES", "f");
  l16 -> AddEntry(Truenergy_elecnu_CCCOH_GENIEnorw, "CC-Coh", "f");
  l16 -> AddEntry(Truenergy_elecnu_CCDIS_GENIEnorw, "CCDIS", "f");
  l16 -> Draw();

  c16->Update();
  c16->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/Truenergy_elecnu_GENIEnorw.pdf");
  c16->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/Truenergy_elecnu_GENIEnorw.eps");
  c16->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/Truenergy_elecnu_GENIEnorw.png");
  c16->Print("/Users/castillofernr/Documents/SBN/GENIE/microboonetunning//output/plots/Truenergy_elecnu_GENIEnorw.C");  
  ...
  hprof_EnuCalo_GENIEnorw->SetLineColor(kGrenergy_elecn);
  hprof_EnuCalo_GENIEnorw->SetLineWidth(2);
  hprof_EnuCalo_GENIEnorw->SetMarkerColor(kGrenergy_elecn);
  ...
  EnuDL_CCMEC_GENIE-> SetFillColor(kGrenergy_elecn);
  ...
  hprof_EnuPan0_GENIE->SetLineColor(kGrenergy_elecn);
  hprof_EnuPan0_GENIE->SetLineWidth(2);
  hprof_EnuPan0_GENIE->SetMarkerColor(kGrenergy_elecn);
  ...
  hprof_EnuPan0_GENIEnorw->SetLineColor(kGrenergy_elecn);
  hprof_EnuPan0_GENIEnorw->SetLineWidth(2);
  hprof_EnuPan0_GENIEnorw->SetMarkerColor(kGrenergy_elecn);
