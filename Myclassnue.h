...
#ifdef Myclassnue_cxx
Myclassnue::Myclassnue(TTree *tree) : fChain(0)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("nue_microB_GiBUU.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("nue_microB_GiBUU.root");
      }
...
Myclassnue::~Myclassnue()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t Myclassnue::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Myclassnue::LoadTree(Long64_t entry)
{
...
void Myclassnue::Init(TTree *tree)
{
...
void Myclassnue::Show(Long64_t entry)
{
...
Int_t Myclassnue::Cut(Long64_t entry)
{
